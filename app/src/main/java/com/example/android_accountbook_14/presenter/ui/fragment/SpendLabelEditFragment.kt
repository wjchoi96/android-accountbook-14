...
import com.example.android_accountbook_14.databinding.FragmentSpendLabelEditBinding
...
import com.example.android_accountbook_14.presenter.viewmodel.SpendLabelEditViewModel
...
@AndroidEntryPoint
class SpendLabelEditFragment : BaseFragment<FragmentSpendLabelEditBinding>() {
    companion object {
        private const val EXTRA_DATA_LABEL_KEY = "label"
        fun getInstance(colorLabel: ColorLabel?): SpendLabelEditFragment = SpendLabelEditFragment().apply {
            arguments = bundleOf(
    ...
    override val layoutResId: Int
        get() = R.layout.fragment_spend_label_edit

    private val viewModel: SpendLabelEditViewModel by viewModels()
    ...
    private fun observeData(){
        ...
        viewModel.modeChanged.observe(viewLifecycleOwner){
            when(it){
                SpendLabelEditViewModel.Mode.Edit -> {
                    binding.btnRegister.text = getString(R.string.btn_modify)
                    binding.tvToolBarTitle.text = getString(R.string.spend_edit_view_edit_mode_title)
                }
                SpendLabelEditViewModel.Mode.NewItem -> {
                    binding.btnRegister.text = getString(R.string.btn_register)
                    binding.tvToolBarTitle.text = getString(R.string.spend_edit_view_title)
                }
                else -> {}
            }
        }
        ...
        viewModel.viewState.observe(viewLifecycleOwner){
            when(it){
                is SpendLabelEditViewModel.SpendLabelEditViewState.FinishEdit -> {
                    requireActivity().onBackPressed()
                }

                is SpendLabelEditViewModel.SpendLabelEditViewState.FailEdit -> {
